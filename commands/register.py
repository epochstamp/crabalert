    ...
    async def register(self, ctx, wallet: str = ""):
        ...
        if "Verified" not in roles_str or "Admin" in roles_str or "Moderator" in roles_str:
            if "Verified" not in roles_str:
                asyncio.create_task(ctx.channel.send(f'Please verify yourself to Crabalert before registering (go to https://discord.gg/PxyXk4TT).'))
            return
        if re.match(WALLET_PATTERN, wallet) and int(wallet, 16) != 0:
            ...
            if rowcount == 0:
                try:
                    ...
                    if status == 1:
                        asyncio.create_task(ctx.channel.send(f'Your wallet {wallet} has been added in the database. Please follow #instructions to proceed to payment.'))
                ...
                if wallet.lower() == data[0][1]:
                    asyncio.create_task(ctx.channel.send(f'Your wallet has already been associated with your Discord ID in our database.'))
                else:
                    asyncio.create_task(ctx.channel.send(f'A wallet ({data[0][1]}) has already been added to our database. If you wish to change it, please open a ticket and provide your new wallet address.'))
            close_database(connection)
        else:
            asyncio.create_task(ctx.channel.send(f'Please provide a valid wallet.'))
