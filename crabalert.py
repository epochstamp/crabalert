...
class Crabalert(commands.Bot):
    ...
    async def on_ready(self):
        if not self._launched:
            ...
            self._refresh_prices_coin_except_tus_loop.start()
            self._launched = True
    ...
    def _fetch_payments_coin_from_aux(self, wallet_address, from_timestamp, contract_address, r):
    ...
    async def _fetch_payments_coin_from(self, wallet_address, from_timestamp, contract_address, previous_number):
    ...
    async def _refresh_tus_loop(self):
        ...
        self._set_variable("price_tus", (float(tus_bot.nick.split(" ")[0][1:])))

    @tasks.loop(minutes=1)
    async def _refresh_prices_coin_except_tus_loop(self):
        await self._refresh_prices_coin_except_tus()

    async def _refresh_prices_coin_except_tus(self):
        global coins
        self._set_variable("price_coins", {c: get_token_price_from_dexs(self._get_variable("web3", lambda: Web3(Web3.HTTPProvider(blockchain_urls["avalanche"]))), "avalanche", c) for c in coins.keys() if c.lower() != TUS_CONTRACT_ADDRESS.lower()})
    ...
    async def _refresh_crabada_transactions_loop(self):
        ...
        if current_block - last_block_seen >= NUMBER_BLOCKS_WAIT_BETWEEN_SNOWTRACE_CALLS:
            self._set_variable("last_block_seen", current_block)
            last_block_crabada_transaction = self._get_variable("last_block_crabada_transaction", lambda: 0)
            ...
            asyncio.create_task(async_http_request_with_callback_on_result(
                    link_transactions,
                    lambda e: self._set_variable("last_block_seen", last_block_seen),
                    TIMEOUT,
                    self._refresh_crabada_transactions,
                    semaphore=self._get_variable(f"sem_{SNOWTRACE_SEM_ID}", lambda: asyncio.Semaphore(value=2))
                )
            )

    async def _refresh_crabada_transactions(self, crabada_transactions):
        ...
        self._set_variable("crabada_transactions", sorted(crabada_transactions, key=lambda x: int(x["blockNumber"])))
        if crabada_transactions != []:
            self._set_variable("last_block_crabada_transaction", int(crabada_transactions[-1]["blockNumber"]) + 1)
    ...
    async def _crabada_alert_loop(self):
        ...
        async with sem:
            ...
            self._set_variable("crabada_transactions", [transaction for transaction in crabada_transactions if transaction["hash"] not in set_transaction_id])
    ...
    async def _handle_crabada_transaction(self, crabada_transaction):
        ...
        asyncio.create_task(
            async_http_request_with_callback_on_result(
                ...
                TIMEOUT,
                lambda r: self._notify_marketplace_item(r, token_id, price, timestamp_transaction),
                semaphore=self._get_variable(f"sem_{APICRABADA_SEM_ID}", lambda: asyncio.Semaphore(value=2))
            )
        )

    async def _notify_marketplace_item(self, r, token_id, price, timestamp_transaction):
        if is_crab(r):
            ...
            asyncio.create_task(
                async_http_request_with_callback_on_result(
                    ...
                    lambda e: None,
                    TIMEOUT,
                    lambda r: self.notify_egg_item(r, token_id, price, timestamp_transaction, channel),
                    semaphore=self._get_variable(f"sem_{APICRABADA_SEM_ID}", lambda: asyncio.Semaphore(value=2)),
                )
            )
    ...
    async def notify_crab_item(self, infos_nft, token_id, price, timestamp_transaction, channel, cashlink=False):
        ...
        if (token_id, timestamp_transaction, channel.id) not in already_seen:
            ...
            if (token_id, timestamp_transaction, channel.id) not in already_seen:
                ...
                task.add_done_callback(lambda t: self._set_variable("already_seen", self._get_variable("already_seen").union({(token_id, timestamp_transaction, channel_id)})))
