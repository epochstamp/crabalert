...
class Crabalert(commands.Bot):
    def __init__(self, command_prefix="!", intents=None, variables=None, crabalert_observers=[]):
        ...
        if variables is None:
            ...
            try:
                ...
                last_block_crabada_transaction = max([block_number_ago]+[int(transaction["blockNumber"]) for transaction in json.loads(urllib.request.urlopen(req).read())["result"] if isinstance(transaction, dict) and is_valid_marketplace_listing_transaction(transaction)])
            except:
                ...
                last_block_crabada_transaction = max([block_number_ago]+[int(transaction["blockNumber"]) for transaction in transactions if isinstance(transaction, dict) and is_valid_marketplace_listing_transaction(transaction)])
            self._variables = {
            ...
            if not "Admin" in roles_str and not "Moderator" in roles_str and ("Verified" in roles_str or "Alerted" in roles_str):
                ...
                if rowcount > 0:
                    ...
                    if current_timestamp - datetime.strptime(start_trial, "%Y-%m-%d %H:%M:%S").astimezone(timezone.utc).timestamp() <= duration_trial:
                        if "Alerted" not in roles_str:
                            task = asyncio.create_task(member.add_roles(role_alerted))
                            asyncio.gather(task)
                        continue
                    else:
                        if "Alerted" in roles_str:
                            asyncio.gather(asyncio.create_task(member.remove_roles(role_alerted)))
                ...
                if rowcount > 0:
                    ...
                    if duration < 0:
                        ...
                        asyncio.gather(asyncio.create_task(self._manage_alert_roles_from_payments(member, wallet_address, payment_timestamp)))
                        """
        ...
        async with self._get_variable(f"sem_{PAYMENT_SEM_ID}_{member.id}_{contract_address}", lambda: asyncio.Semaphore(value=1)):
            ...
            if current_timestamp - int(payment_timestamp) > duration:
                if payments == [] and "Alerted" in roles_str:
                    asyncio.gather(asyncio.create_task(member.remove_roles(role_alerted)))
                    asyncio.gather(asyncio.create_task(member.send("Your subscription to Crabalert has expired and your access to the alerts removed.")))
                    asyncio.gather(asyncio.create_task(member.send("If you want to renew it, please send payments again (see #instructions for a soft reminder)")))
            else:
                ...
                if remaining_time <= 3600*24 and not reminded:
                    ...
                    asyncio.gather(asyncio.create_task(member.send(
                ...
                if "Alerted" not in roles_str:
                    asyncio.gather(asyncio.create_task(member.add_roles(role_alerted)))
            if payments != []:
                for payment, trans_timestamp in payments:
                    ...
                    asyncio.gather(asyncio.create_task(member.send(f"Your payment of {payment} {COINS_SYMBOL.get(contract_address.lower(), '???')} received at {trans_timestamp_date.strftime('%d, %b %Y')} has been checked and your subscription has just been extended for a duration of {human_friendly_duration}.")))
            try:
                close_database(connection)
            except:
                pass
    ...
    async def _manage_alert_roles_from_payments(self, member, wallet_address, payment_timestamp):
        asyncio.gather(asyncio.create_task(async_http_get_request_with_callback_on_result(
    ...
    async def _manage_alert_roles_from_payments_aux(self, member, wallet_address, payment_timestamp, block_number):
        for contract_address in COINS.keys():
            ...
            await async_http_get_request_with_callback_on_result(
    ...
    async def _refresh_crabada_transactions_web3(self, web3, current_block, last_block_crabada_transaction):
        task = asyncio.create_task(
            get_transactions_between_blocks(web3, last_block_crabada_transaction, end_block=current_block, filter_t=lambda t: is_valid_marketplace_listing_transaction(t))
        )
        task.add_done_callback(
    ...
    async def _refresh_crabada_transactions(self, crabada_transactions, current_block):
        self._set_sync_variable("last_block_seen", current_block)
        crabada_transactions_lst = [transaction for transaction in crabada_transactions if isinstance(transaction, dict) and is_valid_marketplace_listing_transaction(transaction)]
        ...
        self._set_sync_variable("crabada_transactions", sorted(crabada_transactions, key=lambda x: int(x["blockNumber"])))
        if crabada_transactions != []:
            self._set_sync_variable("last_block_crabada_transaction", int(crabada_transactions[-1]["blockNumber"]) + 1)
    ...
    async def _crabada_alert_loop(self):
        ...
        self._set_sync_variable("crabada_transactions", [transaction for transaction in crabada_transactions if transaction["hash"] not in set_transaction_id])
    ...
    async def _notify_marketplace_item(self, r, token_id, price, timestamp_transaction, crabada_transaction):
        if is_crab(r):
            for channel_id, filter_function in channel_to_post_listings_with_filters.items():
                ...
                if filter_function((r, None)):
                    asyncio.create_task(
                        self.notify_crab_item(
                            ...
                            channel,
                            shortdescr=channel_id in listing_channels_to_display_shortdescrs
                        )
                    )
    ...
    async def notify_egg_item_channel(self, infos_family_nft, token_id, price, timestamp_transaction, channel):
        async with self._get_variable(f"sem_{EGGMESSAGE_SEM_ID}_{token_id}_{timestamp_transaction}_{channel.id}", lambda: asyncio.Semaphore(value=1)):
            ...
            tus_emoji = channels_emojis.get(channel_id, channels_emojis.get("default")).get("tus", ":tus:")
            crabadegg_emoji = channels_emojis.get(channel_id, channels_emojis.get("default")).get("crabadegg", ":crabadegg:")
            if channel_id in listing_channels_to_display_shortdescrs:
                async with self._get_variable(f"sem_{ADFLY_SEM_ID}_{token_id}_{channel.id}", f_value_if_not_exists=lambda:asyncio.Semaphore(value=1)) as sem:
                    message_egg = (
                        f"{first_column}\n"
                    )
